...
for (i in 1:4) {
  ...
  out <- data.table(Row = rownames(a$coefficients)[-1],
                    Covariate = c("Age (x10)",
                                  ...
                                  "Insurance:Commercial vs. Private",
                                  "Insurance:Medicaid/Self-Pay/Other vs. Private"),
...
p2 <- ggplot(case[ , "los"],
  ...
  geom_histogram(bins = 30,
                 color = "black",
                 fill = "green") +
  scale_x_log10("Length of Stay (Semi-Log Scale)",
...
**NOTE to myself**: Javier wants to eliminate the gaps. How to do it?

```{r los_glm, warning=FALSE}
m1a.1y <- glm(readm1y ~ los,
              family = binomial(logit),
              data = case)
m1b.1y <- glm(hf1y ~ los,
              family = binomial(logit),
              data = case)
m1c.1y <- glm(dead1y ~ los,
              family = binomial(logit),
              data = case)
m1d.1y <- glm(cvdeath1y ~ los,
              family = binomial(logit),
              data = case)
...
datatable(data = t1,
          caption = "Logistic Regression Models of 1-year Outcomes vs. LOS (per day)",
          class = "cell-border stripe",
          rownames = FALSE,
          options = list(searching = TRUE,
                         pageLength = nrow(t1))) %>%
...
```{r reason_for_readm, fig.height = 5, fig.width = 7}
...
hist(dtreadm$days2readm)
hist(log2(dtreadm$days2readm))
...
This model suggests that a patient readmitted for HF within a given time interval from the index HF discharge date will, on average, have his odds of dying within 1 year (from any cause) reduced 27.7% (OR = 0.723, 95% C.I. = 0.717 to 0.729, p-value < 0.001). the time intervals are defined by doubling the amount of days from the previous time interval (as a consequence of log2 transformation of the number of days). For example, the odds of 1 year all-cause mortality of a patient readmitted between days 9 and 16 from the index HF discharge [from day 2^3 + 1 to day 2^4)] will be 27.7% less that the odds of a patient readmitted between days 5 and 8 [from day 2^2 + 1 to 2^3].
...
```{r correlations}
# ${\begin{table}[]\begin{tabular}{llll}& y = 1 & y = 0 & total \\x = 1 & n11 & n10 & n1.   \\x = 0 & n01   & n00   & n0.   \\total & n.1   & n.0   & n    \end{tabular}\end{table}}$
tmp <- case[, haf:htia]
...
p2a <- ggplot(t2a,
  ...
  geom_point(aes(x = names,
             ...
             size = 2,
             shape = 21) +
  scale_fill_manual(labels = paste(1:nlevels(t2a$names),
                                   levels(t2a$names),
                                   sep = ": "),
...
p2c <- ggplot(t2c,
  ...
  geom_point(aes(x = names,
             ...
             size = 2,
             shape = 21) +
  scale_fill_manual(labels = paste(1:nlevels(t2c$names),
                                   levels(t2c$names),
                                   sep = ": "),
...
# Association of 1-Year Readmission with 2-Year All-Cause Mortality
## 2-year all-cause mortality vs. 1-year readmission, unadjusted logistic model
```{r death2y_readm1y_unadj}
# All-cause death within 2 years
case2$dead2y <- FALSE
case2$dead2y[case2$days2death < 2*365.25] <- TRUE

mm1.2y <- glm(dead2y ~ readm1y,
              family = binomial(logit),
              data = case2)
ss1 <-summary(mm1.2y)
...
datatable(out1)
```
Risk of death within 2 years from initial HF discharge was significantly higher in patients who were readmitted for any cause within 1 year from the said discharge (OR = 1.54, 95% C.I. = 1.49 to 1.59, p-value < 0.001).

## 2-year all-cause death vs. 1-year readmission, adjusted for all covariates
```{r death2y_readm1y_adj}
# Remove patients from hosps with no info
tmp <- droplevels(case2[Area != "Unknown", ])
# 3,353 patients with no info about the admission hospital were removed; 89,893 remained

mm2.2y <- glm(dead2y ~ readm1y +
                DSCYR +
                decade +
                SEX +
                RACE +
...
out2 <- data.table(names = c("All-Cause Readmitted within 1 Year",
                             t2.names),
                   coef = round(exp(ss1$coefficients[-1, 1]), 3),
                   lb = round(exp(ss1$coefficients[-1, 1] - 1.96*ss1$coefficients[-1, 2]), 3),
                   ub = round(exp(ss1$coefficients[-1, 1] + 1.96*ss1$coefficients[-1, 2]), 3),
                   ...
                   sign = ifelse(ss1$coefficients[-1, 4] <= 0.05,
                                 ifelse(ss1$coefficients[-1, 4] <= 0.01,
                                        ...
                                        "*"), ""))
datatable(out2,
          caption = "Table: 2-year mortality vs. 1-year readmissions, adjusted model",
          class = "cell-border stripe",
          rownames = FALSE,
          options = list(searching = TRUE,
...
The 2-year risk of death remained significantly higher in patients who were readmitted for any cause within 1 year from the said discharge after adjusting for all covariates (OR = 1.50, 95% C.I. = 1.46 to 1.55, p-value < 0.001).

## 2-year mortality
```{r death2y_vs_readm1y}
...
datatable(tab7,
          caption = "Table: number and percent dead in 2 years",
          class = "cell-border stripe",
          rownames = FALSE,
          options = list(searching = TRUE,
                         pageLength = nrow(table2d)))
...
```{r death2y_vs_readm1y_plot, fig.height = 6,fig.width = 8}
tab7$Alive <- t7[, 1]
tab7$Dead <- t7[, 2]
...
tmp <- melt.data.table(data = tab7,
                       ...
                       variable.name = "2-Year Status",
                       value.name = "Percent")
tmp$`2-Year Status` <- gsub(pattern = "_Pct",
                            replacement = "",
                            x = tmp$`2-Year Status`)
stts <- merge(stts, 
              tmp,
              by = c("Reason",
                     "2-Year Status"))
stts$`2-Year Status` <- factor(stts$`2-Year Status`,
                               levels = c("Dead",
                                          "Alive"))
...
## Odds ratios of 2-year all-cause death by reason for readmission, vs. HF readmission
```{r death2y_vs_readm1y_reason-ref_hf, fig.height = 5, fig.width = 5}
...
datatable(out3,
          caption = "Table: 2-year mortality vs. reason for readmission",
          class = "cell-border stripe",
          rownames = FALSE,
          options = list(searching = TRUE,
...
p4
```
Compare to risk of 2-year (from the index HF discharge) all-cause mortality after the first HF readmission, not being readmitted or being readmitted for the first time for heart disease, respiratory problems, arrhythmia, diabetes, general symptoms, hypertension, procedures or other reasons decreased the risk. On the other hand, readmission for kidney disease or infections increased the risk.

## Odds ratios of 2-year all-cause death by reason for readmission, vs. no readmission
```{r death2y_vs_readm1y_reason-ref_noreadm, fig.height = 5, fig.width = 5}
case2$group <- factor(case2$group,
                      levels = lvls)
