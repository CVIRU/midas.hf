...
NOTE: the higher number in early years could be due to no information on prior HF, hence more patients remained. We should probably start at 2000 with 5-year look-backs.
...
# PART II: Rates
```{r Rates, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE, fig.height=5, fig.width=8}
invisible({
...
# # Because not all subjects will have full follow-up time,
...
# ```
# 
# ```{r PostHoc, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE, fig.height=5, fig.width=5}
# # Part I: all AMIs----
...
# s1 <- summary(m4a.5y)
# 
# t5 <- data.frame(OR = exp(s1$coefficients[-1, 1]),
...
# DT::datatable(round(t5, 3),
#               caption = "Table5: OR of PCI and CABG",
#               rownames = TRUE,
#               options = list(searching = TRUE,
#                              pageLength = nrow(t5)))
# 
...
#               data = case[ADMDAT < "2011-01-01", ])
# s1 <- summary(m4b.5y)
# t6 <- data.frame(OR = exp(s1$coefficients[-1, 1]),
#            `95% C.I.L.L.` = exp(s1$coefficients[-1, 1] - 1.96*s1$coefficients[-1, 2]),
#            `95% C.I.U.L.` = exp(s1$coefficients[-1, 1] + 1.96*s1$coefficients[-1, 2]),
#            `p-Value` = round(s1$coefficients[-1, 4], 4))
# DT::datatable(round(t6, 3),
#               caption = "Table5: OR of PCI and CABG, including AMI admission",
#               rownames = TRUE,
#               options = list(searching = TRUE,
#                              pageLength = nrow(t6)))
# 
...
# # a. Unadjusted----
# m5a.5y <- glm(hf5y ~ sub.ami.dx1,
#               family = binomial(logit),
#               data = case[ADMDAT < "2011-01-01", ])
# s1 <- summary(m5a.5y)
# t7 <- data.frame(OR = exp(s1$coefficients[-1, 1]),
#            `95% C.I.L.L.` = exp(s1$coefficients[-1, 1] - 1.96*s1$coefficients[-1, 2]),
#            `95% C.I.U.L.` = exp(s1$coefficients[-1, 1] + 1.96*s1$coefficients[-1, 2]),
#            `p-Value` = round(s1$coefficients[-1, 4], 4))
# DT::datatable(round(t7, 3),
#               caption = "Table7: OR by Type of AMI, Unadjusted",
#               rownames = TRUE,
#               options = list(searching = TRUE,
#                              pageLength = nrow(t7)))
# 
# # b. Adjusted----
# m5b.5y <- glm(hf5y ~ sub.ami.dx1 +
#                 dschyear +
#                 decade + 
#                 SEX +
#                 HISPAN +
...
# s1 <- summary(m5b.5y)
# 
# t8 <- data.frame(OR = exp(s1$coefficients[-1, 1]),
#            `95% C.I.L.L.` = exp(s1$coefficients[-1, 1] - 1.96*s1$coefficients[-1, 2]),
#            `95% C.I.U.L.` = exp(s1$coefficients[-1, 1] + 1.96*s1$coefficients[-1, 2]),
#            `p-Value` = round(s1$coefficients[-1, 4], 4))
# 
# DT::datatable(round(t8, 3),
#               caption = "Table8: OR by Type of AMI, Adjusted",
#               rownames = TRUE,
#               options = list(searching = TRUE,
#                              pageLength = nrow(t8)))
```