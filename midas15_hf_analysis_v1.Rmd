...
out <- data.table(bp = round(2000 + m1.1$psi[2], 1),
...
The breakpoint was estimated to be at the year `r out$bp` (95% C.I.  = `r out$bplb` to `r out$bpub`). The number of new HF patients dicreased significantly between the years 2000 and 2006 by `r -out$slope1` patients on average (95% C.I. = `r out$s1lb` to `r out$s1ub`, p-value < 0.001), and remained almost unchanged from 2007 onward.      
...
names(out) <- names(t1.apx)
out
```

# Length of stay
```{r length_of_stay, warning=FALSE}
# Length of stay
...
```{r reason_for_readm, fig.height = 8, fig.width = 10}
...
## Odds ratios of 2-year all-cause death by reason for readmission
```{r death2y_vs_readm1y_reason, fig.height = 5, fig.width = 5}
