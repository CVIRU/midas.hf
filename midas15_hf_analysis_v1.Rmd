...
p2b <- ggplot(t2b,
  ...
  geom_point(aes(x = names,
             ...
             size = 2,
             shape = 21) +
  scale_fill_manual(labels = paste(1:nlevels(t2b$names),
                                   levels(t2b$names),
                                   sep = ": "),
...
p2d <- ggplot(t2d,
  ...
  geom_point(aes(x = names,
             ...
             size = 2,
             shape = 21) +
  scale_fill_manual(labels = paste(1:nlevels(t2d$names),
                                   levels(t2d$names),
                                   sep = ": "),
...
```{r death2y_readm1y_adj, fig.height = 5, fig.width = 15}
# Remove patients from hosps with no info
...
datatable(data = out2,
          caption = "Table: 2-year cause of death vs. 1-year readmission, adjusted",
          class = "cell-border stripe",
          rownames = FALSE,
...
p1 <- ggplot(out2,
  ...
  geom_point(aes(x = names,
             ...
             size = 2,
             shape = 21) +
  scale_fill_manual(labels = paste(1:nlevels(out2$names),
                                   levels(out2$names),
                                   sep = ": "),
  ...
  scale_y_continuous("Odds Ratios") +
  ggtitle("Odds Ratios of 2-Year All-Cause Death") +
  theme(axis.text.x = element_text(angle = 90,
                                   size = 12,
                                   hjust = 1,
                                   vjust = 0.5),
...
The 2-year risk of death remained significantly higher in patients who were readmitted for any cause within 1 year from the said discharge after adjusting for all covariates (OR = 1.49, 95% C.I. = 1.45 to 1.54, p-value < 0.001).
