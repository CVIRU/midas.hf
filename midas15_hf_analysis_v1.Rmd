...
```{r readm_reason_grp}
dt1 <- fread("data/HF readmissions by cause.csv",
             header = TRUE)
dt1
```
...
# Additional Analysis: effect of readmission for specific reason
```{r readm_cause_grp}
readm_grp <- fread("data/HF readmissions by cause.csv")

readm_grp <- merge(readm_grp,
                   readm_cause,
                   by = "major")
readm_grp <- readm_grp[, c("Patient_ID",
                           "group")]
table(readm_grp$group)

case1 <- merge(case, 
               readm_grp,
               by = "Patient_ID",
               all.x = TRUE)
case1$group[is.na(case1$group)] <- "Other Reason"
case1$group[case1$days2readm > 365] <- "Not Readmitted"
table(case1$group)

case$dead1y[case$days2death < 366] <- TRUE
case1$group <- factor(case1$group,
                      levels = c("Not Readmitted",
                                 unique(case1$group)[-5]))
