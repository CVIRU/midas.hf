...
# Additional Analysis: Effect of Readmission for Specific Reason on 1-Year All-Cause Mortality
...
datatable(readm_grp,
          options = list(pageLength = nrow(readm_grp)))
...
case1$dead1y <- FALSE
case1$dead1y[case1$days2death < 366] <- TRUE
...
case2$dead1y[case2$days2death < 366] <- TRUE
case2$dead1y[is.na(case2$dead1y)] <- FALSE

tab7 <- addmargins(table(case2$group,
                         case2$dead1y))

tab7b <- data.table(Reason = rownames(tab7),
                    Alive = format(tab7[, 1],
                                   big.mark = ","),
                    Alive_Pct = round(100*tab7[, 1]/tab7[, 3], 1),
                    Dead = format(tab7[, 2],
                                  big.mark = ","),
                    Dead_Pct = round(100*tab7[, 2]/tab7[, 3], 1),
                    Total = format(tab7[, 3],
                                     big.mark = ","))

datatable(tab7b,
          options = list(pageLength = nrow(tab7b)))


mm1.1y <- glm(dead1y ~ group,
              family = binomial(logit),
              data = case2)
...
out <- data.table(names = levels(case2$group)[-1],
                  ...
                  sign = ifelse(ss1$coefficients[-1, 4] <= 0.05,
                                ifelse(ss1$coefficients[-1, 4] <= 0.01,
                                       ...
                                       "*"), ""))
datatable(out,
          options = list(pageLength = nrow(out)))


out$names <- factor(out$names,
                    levels = rev(levels(case2$group)[-1]))
p7 <- ggplot(out,
              aes(x = names,
                  y = coef)) +
  coord_flip() +
...
p7
```
Compare to risk of 1-year (from the index HF discharge) all-cause mortality after the first HF readmission, not being readmitted or being readmitted for the first time for heart disease, respiratory problems, arrhythmia, diabetes, general symptoms, hypertension, procedures or other reasons decreased the risk. on the other hand, readmission for kidney disease, infections or cerebrovascular issues increased the risk.
