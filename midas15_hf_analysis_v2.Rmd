---
title: "Cardiovascular Outcomes in Heart Failure Patients with Existing Hypertension"
output:
  html_notebook:
    highlight: tango
    number_sections: yes
...
# Keep only the hypertensive patients
case <- droplevels(case[hhyper == TRUE, ])
case$hhyper <- NULL
...
t2 <- t1[Year > 1999 &
           Year < 2015, ]
# Plot HF discharges----
p0ttl <- paste("Figure 1: Number of first HF discharges. Total of",
...
# Demographics 
```{r demog}
# Pubication/Table2: Demographics----
# summary(case)
table2 <- data.table(Description = c("Mean Age at First HF Admission +/- S.D",
                                     "Male (%)",
                                     "Race (%)",
                                     "* White",
                                     "* Black",
                     ...
                     `Number of Patients` = c(paste(round(mean(case$AGE), 1),
                                              ...
                                              format(nrow(case),
                                                     big.mark = ",")),
                     `Percent Patients` = c("",
                                            round(100*sum(case$SEX == "M")/nrow(case),
                                                  1),
                                            "",
...
datatable(table2,
          ...
          class = "cell-border stripe",
          options = list(searching = FALSE,
                         pageLength = nrow(table2)))
```


# Comorbidities
```{r table2comorb}
table2comorb <- data.table(Description = c("Prior AF",
                                           "Prior AMI",
                                           "History of Anemia",
                                           ...
                                           "Prior Stroke",
                                           "Prior TIA"),
                           `Number of Patients` = c(format(sum(case$haf),
                                                           big.mark = ","),
                                                    format(sum(case$hami),
                                                           big.mark = ","),
                                                    format(sum(case$hanemia),
                                                           big.mark = ","),
                                                    format(sum(case$hckd),
                                                           big.mark = ","),
                                                    format(sum(case$hcopd),
                                                           big.mark = ","),
                                                    format(sum(case$hdiab),
                                                           big.mark = ","),
                                                    format(sum(case$hlipid),
                                                           big.mark = ","),
                                                    format(sum(case$hosa),
                                                           big.mark = ","),
                                                    format(sum(case$hpark),
                                                           big.mark = ","),
                                                    format(sum(case$hstroke),
                                                           big.mark = ","),
                                                    format(sum(case$htia),
                                                           big.mark = ",")),
                           `Percent Patients` = c(round(100*sum(case$haf)/nrow(case), 1),
                                                  round(100*sum(case$hami)/nrow(case), 1),
                                                  round(100*sum(case$hanemia)/nrow(case), 1),
                                                  round(100*sum(case$hckd)/nrow(case), 1),
                                                  round(100*sum(case$hcopd)/nrow(case), 1),
                                                  round(100*sum(case$hdiab)/nrow(case), 1),
                                                  round(100*sum(case$hlipid)/nrow(case), 1),
                                                  round(100*sum(case$hosa)/nrow(case), 1),
                                                  round(100*sum(case$hpark)/nrow(case), 1),
                                                  round(100*sum(case$hstroke)/nrow(case), 1),
                                                  round(100*sum(case$htia)/nrow(case), 1)))
# Save Table2comorb as CSV
write.csv(table2comorb,
          file = "tmp/table2comorb.csv",
          row.names = FALSE)

datatable(table2comorb,
          caption = "Table 2 Comorb: HF patients' comorbidities",
          rownames = FALSE,
          class = "cell-border stripe",
          options = list(searching = FALSE,
                         pageLength = nrow(table2comorb)))
```
...
# Subanalysis: anudjusted analysis by age groups
```{r age_grp}
case$age_grp <- "<75"
case$age_grp[case$AGE >= 75] <- "75 to 84"
case$age_grp[case$AGE >= 85] <- ">=85"
case$age_grp <- factor(case$age_grp,
                       levels = c("<75",
                                  "75 to 84",
                                  ">=85"))

m1a1.1y <- glm(readm1y ~ age_grp,
               family = binomial(logit),
               data = case)

m1b1.1y <- glm(hf1y ~ age_grp,
               family = binomial(logit),
               data = case)

m1c1.1y <- glm(dead1y ~ age_grp,
               family = binomial(logit),
               data = case)

m1d1.1y <- glm(cvdeath1y ~ age_grp,
               family = binomial(logit),
               data = case)

s11 <- list(summary(m1a1.1y),
            summary(m1b1.1y),
            summary(m1c1.1y),
            summary(m1d1.1y))
...
for (i in 1:4) {
  a <- s11[[i]]
  t1[[i]] <- data.table(Outcome = outcomes[i],
                        Row = rownames(a$coefficients)[-1],
                        OR = exp(a$coefficients[-1, 1]),
                        `95% C.I.L.B.` = exp(a$coefficients[-1, 1] - 1.96*a$coefficients[-1, 2]),
                        `95% C.I.U.B.` = exp(a$coefficients[-1, 1] + 1.96*a$coefficients[-1, 2]),
                        `p-Value` = ifelse(a$coefficients[-1, 4] >= 0.001,
...
write.csv(out,
          file = "tmp/outcomes_by_age_grp.csv")

print(datatable(data = out,
                caption = "Table: Subanalysis: Outcomes by Age Group",
                class = "cell-border stripe",
                rownames = FALSE,
                options = list(searching = FALSE,
                               pageLength = nrow(out))) %>%
...
**NOTE***: 1,943  patients were discharged at the day of admission. This might include transfers to other hospitals but we did not look at transfers.
...
This model suggests that a patient readmitted for HF within a given time interval from the index HF discharge date will, on average, have their odds of dying within 1 year (from any cause) reduced by 18.4% (OR = 0.816, 95% C.I. = 0.805 to 0.827, p-value < 0.001). The time intervals are defined by doubling the amount of days from the previous time interval (as a consequence of log2 transformation of the number of days). For example, the odds of 1 year all-cause mortality of a patient readmitted between days 9 and 16 from the index HF discharge (i.e. between [day 2^3^ + 1] and day 2^4^) will be 18.4% less than the odds of a patient readmitted between days 5 and 8 (i. e. between day [2^2^ + 1] and day 2^3^).
...
datatable(data = out1,
  ...
  formatRound(columns = 2:4,
              digits = 3)
```
Risk of death within 3 years from initial HF discharge was significantly higher in patients who were readmitted for any cause within 1 year from the said discharge (OR = 1.31, 95% C.I. = 1.26 to 1.36, p-value < 0.001).
